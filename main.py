...
llm = ChatGroq(temperature=0, groq_api_key=os.getenv("GROQ_API_KEY"), model_name="llama-3.2-90b-text-preview")
...
def runQuery(query):
    try:
        ...
        st.error(f"SQL query execution failed: {error_message}")
    ...
    # Detect if it's a missing column issue
        if 'Unknown column' in error_message:
            ...
            st.warning(f"The specified column '{missing_column}' does not exist in the table. Please verify your column names.")
    # Detect if it's a missing table issue
            ...
            st.warning(f"The specified table '{missing_table}' does not exist in the database. Please check your query and try again.")
        else:
            st.error("There was an issue with your query. Please review the syntax and try again.")
